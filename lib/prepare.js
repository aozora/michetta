...
const indexRE = /\b(index|readme)\.(vue|md)$/i;
const extRE = /\.(vue|md)$/;
...
async function resolveOptions(sourceDir) {
  ...
  const options = {
    ...
    layoutTemplate: siteConfig.layout ? path.resolve(sourceDir, siteConfig.layout) : path.resolve(__dirname, './app/default.html'),
    pageTemplatePath: siteConfig.pageTemplate ? path.resolve(sourceDir, siteConfig.pageTemplate) : path.resolve(__dirname, './app/Page.vuw'),
    components: path.resolve(sourceDir, 'components'),
    outDir: siteConfig.dest
      ? path.resolve(siteConfig.dest)
      : path.resolve(sourceDir, '.vuepress/dist'),
    publicPath: base,
    pageFiles: sort(await globby(['pages/**/*.vue', '!.vuepress', '!node_modules'], { cwd: sourceDir })),
    pagesData: null,
  ...
  const pagesData = await Promise.all(options.pageFiles.map(async (file) => {
    const data = {
      ...
      componentPath: path.resolve(options.sourceDir, file),
